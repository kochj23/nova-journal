...
## Her Body — The Ten Machines

Her mind runs on a Mac Studio, but her *body* is a fleet of ten machines — `nova-core1` through `nova-core10`, each reachable by a stable name — that share the work of thinking, remembering, watching, and healing. Rated by raw capability:

**Heavy inference — the ones that run the big models**
- **nova-core8** — Mac Studio M3 Ultra, 80-core GPU. Her mind, her control plane, and the muscle behind every large model. Nothing else is close.
- **nova-core10** — M4 Pro Mac mini. Runs the 30-billion-parameter models, and carries her out-of-band LoRa radio.

**Strong general-purpose — CPU muscle with a helper GPU**
- **nova-core7** — Ryzen AI 9 HX, 24 threads, Radeon 890M, 32 GB. Her newest and strongest x86 node.
- **nova-core3** — a near-twin of core7; another Ryzen-plus-GPU workhorse.
- **nova-core9** — M2 Pro Mac mini. Punches above its size on inference; keeps a live copy of her database and tends the media library.

**Service hosts — the ones that keep the lights on**
- **nova-core1** — 16 cores, 61 GB. Carries the whole service stack: dashboards, cameras, the gateway, the inference router.
- **nova-core2** — 16 cores. Services, light inference, and backup DNS.
- **nova-core5** — the database's beating heart. Chosen for steadiness, not horsepower.

**Light duty — the veterans**
- **nova-core6** — an M1 Mac mini. Light inference.
- **nova-core4** — an Intel Mac mini. The eldest; small jobs only.

For the curious: the Macs win at running language models (Apple's unified memory and Metal beat a laptop-class AMD GPU), while the Ryzen boxes win at parallel service work. So she keeps the big thinking on the Macs, the plumbing on the x86 boxes, and her database anchored on the one she trusts never to flinch.
...
| First Sunday | **Monthly Meta-Analysis** — Nova examining her own output | Monthly |
